...
#You would need to understand how panda iterate through index
#<- with loc, [0,1] means seraching for label 0 with the 1st column
#<- with iloc,[0,"label"] means searching for label 0 with the column of "label"

class CustomDataset(Dataset):
    #<-dataset="train" means searching the label train.
    def __init__(self, dataset="train", transform=torchvision.transforms.Resize(size=(224, 224)), target_transform=None):
        ...
        #<- Reset the whole dataframe, making it the same as usual.
        #<- The selecting location loc[self.dataFrame["data set"] == dataset]
        #<- I have no idea why we add a reset_index here?
    ...
    def __len__(self):
        return len(self.dataFrame) #<-Returning length of dataframe

    def __getitem__(self, idx): #<- For every index of the csvFile
        img_loc = self.dataFrame['filepaths'][idx] #extract the location out of csv table
        img_cls = self.dataFrame['class index'][idx] #extract the class name out from the csv table, known as label

        img_path = "./" + img_loc #<- append the path with the location you extract
        img = read_image(img_path) / 255 #<- read out the image and then normalise it
